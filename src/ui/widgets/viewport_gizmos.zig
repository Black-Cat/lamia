...
const InteractionInfo = struct {
    draw_list: *nc.ImDrawList,

    camera: *Camera,
    viewport: nm.vec4,

    center: nm.vec2,
    original_point: nm.vec2,
    center3: nm.vec3,
    original_point3: nm.vec3,
    direction: nm.vec3,
    ...
    original_value: f32,
    original_value3: nm.vec3,

    value: *f32,
    value3: *nm.vec3,

    interactFn: ?fn (interaction_info: *InteractionInfo) void,
};
...
pub fn drawGizmos(gizmos: *GizmoStorage, camera: *Camera, min_pos: nc.ImVec2, max_pos: nc.ImVec2, window_pos: nc.ImVec2) void {
    if (gizmos.points_to_transform.items.len == 0)
        return;

    var interaction_info: InteractionInfo = undefined;
